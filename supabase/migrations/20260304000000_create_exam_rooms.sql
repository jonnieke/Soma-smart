...
-- Add foreign key back to exam_rooms for pinned message (idempotent)
DO $$ BEGIN IF NOT EXISTS (
    SELECT 1
    FROM information_schema.table_constraints
    WHERE constraint_name = 'fk_exam_rooms_pinned_msg'
...
-- 5. RPC Functions for member counts
CREATE OR REPLACE FUNCTION public.increment_room_member(row_id UUID) RETURNS void AS $$ BEGIN
UPDATE public.exam_rooms
SET member_count = member_count + 1
WHERE id = row_id;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
CREATE OR REPLACE FUNCTION public.decrement_room_member(row_id UUID) RETURNS void AS $$ BEGIN
UPDATE public.exam_rooms
...
-- Exam Rooms Policies
DROP POLICY IF EXISTS "Anyone can view active exam rooms" ON public.exam_rooms;
CREATE POLICY "Anyone can view active exam rooms" ON public.exam_rooms FOR
SELECT USING (is_active = true);
DROP POLICY IF EXISTS "Users can create exam rooms" ON public.exam_rooms;
CREATE POLICY "Users can create exam rooms" ON public.exam_rooms FOR
INSERT WITH CHECK (auth.uid() IS NOT NULL);
DROP POLICY IF EXISTS "Creators can update exam rooms" ON public.exam_rooms;
...
DROP POLICY IF EXISTS "Anyone can view room members" ON public.exam_room_members;
CREATE POLICY "Anyone can view room members" ON public.exam_room_members FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Users can join rooms" ON public.exam_room_members;
CREATE POLICY "Users can join rooms" ON public.exam_room_members FOR
INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can leave rooms" ON public.exam_room_members;
...
DROP POLICY IF EXISTS "Anyone can view messages" ON public.exam_room_messages;
CREATE POLICY "Anyone can view messages" ON public.exam_room_messages FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Members can insert messages" ON public.exam_room_messages;
CREATE POLICY "Members can insert messages" ON public.exam_room_messages FOR
INSERT WITH CHECK (
        auth.uid()::text = user_id
        OR user_id = 'system'
    );
DROP POLICY IF EXISTS "Users can update own messages" ON public.exam_room_messages;
...
DROP POLICY IF EXISTS "Anyone can view votes" ON public.exam_room_votes;
CREATE POLICY "Anyone can view votes" ON public.exam_room_votes FOR
SELECT USING (true);
DROP POLICY IF EXISTS "Users can insert votes" ON public.exam_room_votes;
CREATE POLICY "Users can insert votes" ON public.exam_room_votes FOR
INSERT WITH CHECK (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can update own votes" ON public.exam_room_votes;
CREATE POLICY "Users can update own votes" ON public.exam_room_votes FOR
UPDATE USING (auth.uid() = user_id);
DROP POLICY IF EXISTS "Users can delete own votes" ON public.exam_room_votes;
