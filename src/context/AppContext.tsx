...
export const AppProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  const syncLearningCredits = async (profileId: string, studentId?: string | null) => {
    ...
    if (!isUuid) {
      // Local/Guest learner without Supabase auth: load from localStorage
      const localCredits = sanitizeLearningCredits(parseInt(localStorage.getItem('soma_learning_credits') || '0', 10));
      setLearningCredits(localCredits);
      return;
    }
