    ...
    return (
        <div className="space-y-8">
            ...
            {decoratedStudents.length > 0 && (() => {
                const topPerformer = [...decoratedStudents].sort((a, b) => b.averageScore - a.averageScore)[0];
                const activeCount = decoratedStudents.filter(s => s.lastActive !== 'Recently' ? (() => { try { return (Date.now() - new Date(s.lastActive).getTime()) < 7 * 864e5; } catch { return false; } })() : true).length;
                const digestText = [
                    `📊 *${selectedClass} — Weekly Digest*`,
                    ``,
                    `👩‍🎓 Students: ${decoratedStudents.length}`,
                ...
                return (
                    <div className="bg-gradient-to-r from-indigo-600 to-violet-600 rounded-[2rem] p-5 text-white flex items-center gap-4 shadow-lg shadow-indigo-200/50">
                        <div className="flex-1 min-w-0">
                            <p className="text-[10px] font-black uppercase tracking-widest text-indigo-200 mb-1">Weekly Digest</p>
                            <p className="text-sm font-black leading-snug">
                                {activeCount}/{decoratedStudents.length} active · Avg {classAverage}% · {mostCommonWeakTopic && <>📌 {mostCommonWeakTopic}</>}
                            </p>
                            {topPerformer && <p className="text-xs font-bold text-indigo-200 mt-0.5">🏆 Top: {topPerformer.name}</p>}
                        </div>
                        <a
                            href={`https://wa.me/?text=${encodeURIComponent(digestText)}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            className="shrink-0 flex items-center gap-2 bg-white/20 hover:bg-white/30 border border-white/20 px-4 py-2.5 rounded-xl text-xs font-black transition-colors whitespace-nowrap"
                        >
