...
const normalizeExamType = (value: unknown): 'KCSE' | 'KPSEA' | 'KJSEA' | 'OTHER' => {
    ...
    if (normalized === 'KPSEA' || normalized === 'KPSEA_STYLE' || normalized === 'KPSEA-STYLE' || normalized === 'KPSEA STYLE') return 'KPSEA';
...
export const ExamsView: React.FC = () => {
    ...
    const handleAnalyze = async () => {
        ...
        try {
            ...
            if (inputMode === 'STRUCTURED_JSON') {
                ...
                const resolvedExamType = normalizeExamType(examPayload.examType ?? examPayload.exam_type ?? examType);
    ...
    const validateExamPackage = () => {
        ...
        const sectionValues = Array.from(new Set(questions.map(question => String(question.section || '').trim().toUpperCase()).filter(Boolean)));

        const hasRomanSections = sectionValues.some(section => ['I', 'II', 'III', 'IV', 'V'].includes(section));

        const sectionOneCount = questions.filter(question => String(question.section || '').trim().toUpperCase() === 'I').length;

        const sectionTwoCount = questions.filter(question => String(question.section || '').trim().toUpperCase() === 'II').length;
        ...
        if (questions.some(question => !String(question.text || '').trim())) issues.push('Every question needs question text.');

        if (questions.some(question => !Number(question.marks || 0))) issues.push('Every question needs marks.');

        if (hasRomanSections && sectionTwoCount > 0 && sectionTwoCount < 5) issues.push('Section II should contain at least 5 questions for the learner drill flow.');

        if (hasRomanSections && questions.some(question => String(question.section || '').trim().toUpperCase() === 'II') && sectionOneCount === 0) issues.push('Section I should be present for a real exam layout.');

        if (Number(totalMarks) > 0 && totalQuestionMarks > 0 && Math.abs(totalQuestionMarks - Number(totalMarks)) > 5) {

            issues.push(`Question marks total ${totalQuestionMarks}, but the paper is set to ${Number(totalMarks)}.`);
                                    ...
                                    <div className="min-w-0">
                                        ...
                                        <div className="mt-2 flex flex-wrap gap-2">
                                            ...
                                            {(exam.source === 'STRUCTURED_IMPORT' || /mock/i.test(exam.title)) && (
                                                <span className="rounded-full bg-violet-50 px-2.5 py-1 text-[10px] font-bold text-violet-700">Original mock</span>
                                            )}
                                ...
                                {!analyzedData ? (
                                        ...
                                        {inputMode === 'STRUCTURED_JSON' && (
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <textarea
                                                    ...
                                                    onChange={event => { setStructuredJson(event.target.value); setStructuredJsonFile(null); setJsonImportError(null); }}
                                    ...
                                    <div className="mt-6 space-y-5">
                                        ...
                                        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                                            ...
                                            <p className="text-xs font-black uppercase tracking-wider text-slate-500">Publish checks</p>

                                            <div className="mt-3 space-y-2 text-sm text-slate-600">

                                                {validateExamPackage().length === 0 ? (

                                                    <p className="font-semibold text-emerald-700">Everything looks ready for learners.</p>
...
const FilePicker: React.FC<{
    ...
    <div className="rounded-2xl border-2 border-dashed border-slate-200 bg-slate-50 p-6 text-center hover:border-indigo-300">
        ...
        <input id={id} type="file" className="hidden" accept=".pdf,image/*,.json" onChange={event => onChange(event.target.files?.[0] || null)} />
