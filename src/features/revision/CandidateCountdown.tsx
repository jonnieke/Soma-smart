...
export const CandidateCountdown: React.FC = () => {
    ...
    return (
        <div className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden transition-all group">
                ...
                <div className="w-full md:w-1/2 p-6 sm:p-8 bg-white flex flex-col justify-between">
                    <div>
                        <div className="flex items-center justify-between mb-6">
                            <div className="flex items-center gap-2 bg-amber-50 text-amber-600 px-3 py-1 rounded-full border border-amber-100">
                            ...
                            <div className="flex gap-1">
                                {exams.map((_, i) => (
                                    <button
                                        ...
                                        className={`w-2 h-2 rounded-full transition-all ${activeIndex === i ? 'bg-indigo-600 w-4' : 'bg-slate-200'}`}
                                    />
                                ))}
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            <div className="flex items-start gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100">
                                ...
                                <div>
                                    <p className="text-xs font-bold text-slate-800">Priority Areas</p>
                                    <p className="text-[10px] text-slate-500">Based on previous 5-year patterns.</p>
                                </div>
                            </div>
                        </div>
                    </div>

                    <button className="mt-8 w-full bg-slate-900 text-white py-3 rounded-2xl font-black text-xs uppercase tracking-widest flex items-center justify-center gap-2 group hover:gap-4 transition-all shadow-xl shadow-slate-200">
