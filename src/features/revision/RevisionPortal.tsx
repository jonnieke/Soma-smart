            ...
            <main className="mx-auto max-w-6xl space-y-20 px-4 py-10 sm:px-8 sm:py-16">
                <section className="text-center">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl space-y-6">
                        ...
                        <h1 className="text-5xl font-black leading-[1.05] tracking-tight sm:text-7xl">Past papers first.<br /><span className="bg-gradient-to-r from-orange-500 to-rose-500 bg-clip-text text-transparent">Open one and start improving.</span></h1>
                ...
                <section className="rounded-[2.5rem] border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 sm:p-10">
                    <div className="mb-8 flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600">Paper library</p><h2 className="mt-2 text-3xl font-black">{pathway} papers ready to open</h2><p className="mt-2 text-sm text-slate-500">Open a real paper first, then use the rest of the library to practise more.</p></div><button onClick={openExamLibrary} className="hidden items-center gap-2 text-sm font-black text-indigo-600 sm:flex">View all <ArrowRight className="h-4 w-4" /></button></div>
                        ...
                        <div className="grid gap-4 md:grid-cols-3">
                            {pathwayExams.slice(0, 3).map(exam => (
                                <button key={exam.id} onClick={() => startRevision(exam.id)} className="group rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-white dark:border-slate-800 dark:bg-slate-950">
                                    ...
                                    <span className="mt-4 inline-flex items-center gap-2 text-xs font-black text-indigo-600 group-hover:translate-x-0.5 transition-transform">Open paper <ArrowRight className="h-3.5 w-3.5" /></span>
                                </button>
                            ))}
                        </div>
