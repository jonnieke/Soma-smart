...
export const RevisionPortal: React.FC = () => {
    ...
    const pathwayExams = useMemo(() => publishedExams.filter(exam => {
        ...
        return (examType === pathway || String(exam.grade || '').toLowerCase() === pathwayConfig.grade.toLowerCase()) && isSomaOriginalExam(exam);
            ...
            <main className="mx-auto max-w-6xl space-y-20 px-4 py-10 sm:px-8 sm:py-16">
                <section className="text-center">
                    <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="mx-auto max-w-4xl space-y-6">
                        ...
                        <h1 className="text-5xl font-black leading-[1.05] tracking-tight sm:text-7xl">SomaAI Originals first.<br /><span className="bg-gradient-to-r from-orange-500 to-rose-500 bg-clip-text text-transparent">Open one and start improving.</span></h1>
                        <p className="mx-auto max-w-2xl text-lg font-medium leading-relaxed text-slate-600 dark:text-slate-300">Choose a real SomaAI original, open it fast, and use marks and feedback to recover weak areas before exam day.</p>
                        ...
                        {pathwayExams.length > 0 && <div className="grid gap-3 pt-2 sm:grid-cols-3">{pathwayExams.slice(0, 3).map(exam => <button key={exam.id} onClick={() => startRevision(exam.id)} className="rounded-2xl border border-slate-200 bg-white px-4 py-4 text-left transition hover:-translate-y-0.5 hover:border-indigo-200 hover:shadow-lg dark:border-slate-700 dark:bg-slate-900"><span className="inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-700">Ready original</span><span className="mt-3 block text-sm font-black leading-snug">{exam.title}</span><span className="mt-2 block text-[11px] font-bold text-slate-400">{exam.subject} ? {exam.grade}</span></button>)}</div>}
                        <div className="flex flex-wrap justify-center gap-5 text-xs font-bold text-slate-500"><span className="flex items-center gap-2"><ShieldCheck className="h-4 w-4 text-indigo-500" /> Originals first</span><span className="flex items-center gap-2"><BookOpen className="h-4 w-4 text-indigo-500" /> {publishedExams.length} published exam{publishedExams.length === 1 ? '' : 's'}</span></div>
                    </motion.div>
                </section>

                <section className="rounded-[2.5rem] border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900 sm:p-10">
                    <div className="mb-8 flex items-end justify-between gap-4"><div><p className="text-xs font-black uppercase tracking-[0.2em] text-indigo-600">Originals library</p><h2 className="mt-2 text-3xl font-black">{pathway} originals ready to open</h2><p className="mt-2 text-sm text-slate-500">Start with one original, then move through the rest at your pace.</p></div><button onClick={openExamLibrary} className="hidden items-center gap-2 text-sm font-black text-indigo-600 sm:flex">View all <ArrowRight className="h-4 w-4" /></button></div>
                    {loadingExams ? <div className="flex justify-center gap-2 py-12 text-sm font-bold text-slate-400"><Loader2 className="h-5 w-5 animate-spin" /> Loading published originals...</div> : pathwayExams.length ? <div className="space-y-6">
                        <div className="grid gap-4 md:grid-cols-3">
                            {pathwayExams.slice(0, 3).map(exam => (
                                <button key={exam.id} onClick={() => startRevision(exam.id)} className="group rounded-2xl border border-slate-200 bg-slate-50 p-5 text-left transition hover:-translate-y-0.5 hover:border-indigo-200 hover:bg-white dark:border-slate-800 dark:bg-slate-950">
                                    <span className="inline-flex rounded-full bg-emerald-50 px-2 py-0.5 text-[9px] font-black uppercase tracking-wider text-emerald-700">Ready original</span>
                                    ...
                                    <span className="mt-3 flex flex-wrap gap-2 text-[10px] font-bold text-slate-500">
                                        {exam.duration_minutes && <span className="rounded-full bg-slate-100 px-2 py-1 dark:bg-slate-800">{exam.duration_minutes} min</span>}
