...
export const LearnerAnalytics: React.FC<LearnerAnalyticsProps> = ({
    ...
    return (
        <div className="bg-slate-50 min-h-screen p-6 w-full animate-in fade-in duration-300">
            <div className="w-full max-w-6xl mx-auto space-y-8">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        ...
                        <p className="text-slate-500 font-medium">Track your academic progress and mastery.</p>
                    </div>
                    <div className="bg-indigo-50 border border-indigo-100 px-6 py-3 rounded-2xl flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-[10px] uppercase tracking-widest font-black text-indigo-500">Current Level</p>
                            <p className="text-xl font-black text-slate-800">Level {levelInfo.level}</p>
                        </div>
                    </div>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <Card className="p-5 border-l-4 border-l-amber-500">
                        ...
                        <p className="text-3xl font-black text-slate-800">{totalXP.toLocaleString()}</p>
                    ...
                    <Card className="p-5 border-l-4 border-l-emerald-500">
                        ...
                        <p className="text-3xl font-black text-slate-800">{hrs}h {mins}m</p>
                    ...
                    <Card className="p-5 border-l-4 border-l-purple-500">
                        ...
                        <p className="text-3xl font-black text-slate-800">{questionsAsked}</p>
                    ...
                    <Card className="p-5 border-l-4 border-l-rose-500">
                        ...
                        <p className="text-3xl font-black text-slate-800">{quizzesTaken}</p>
                        ...
                        {(() => {
                            ...
                            return (
                                <>
                                        ...
                                        {CBC_SUBJECTS.map(subj => {
                                            ...
                                            return (
                                                <motion.div
                                                    ...
                                                    className={`rounded-2xl p-2.5 text-center border-2 ${cbc ? cbc.cls : 'bg-slate-50 border-slate-200'}`}
                                                >
                                    ...
                                    {weakest && (
                                        <div className="flex items-center gap-3 bg-rose-50 border border-rose-200 rounded-2xl p-3 mt-2">
                            ...
                            {weakTopics.length > 0 ? (
                                <div className="space-y-3">
                                    ...
                                    {weakTopics.map((topic, i) => (
                                        <div key={i} className="flex items-center gap-3 bg-white/60 p-3 rounded-xl border border-white shadow-sm">
                                            <div className="w-2 h-2 rounded-full bg-rose-500 shrink-0" />
                                            <span className="text-sm font-bold text-slate-700 line-clamp-1">{topic}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
                            ...
                            <div className="space-y-4">
                                {recentActivity.length > 0 ? recentActivity.map((item, i) => (
                                    <div key={i} className="flex items-start gap-4 flex w-full">
                                        <div className="w-8 h-8 rounded-full bg-slate-100 flex items-center justify-center shrink-0 border-2 border-slate-300 mt-1">
                                            {item.type === 'QUIZ' ? <PenTool className="w-3 h-3 text-rose-500" /> : <Lightbulb className="w-4 h-4 text-amber-500" />}
                                        </div>
                                        <div className="flex-1 w-full overflow-hidden">
                                            <p className="text-sm font-bold text-slate-700 truncate w-full block">{item.topic || 'General Revision'}</p>
